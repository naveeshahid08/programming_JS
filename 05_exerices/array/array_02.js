...
let newPlanets = planets.forEach((planet, index) =>
...
wowDatatypes.forEach((dataTypes, index, object) =>
  console.log(`Item:${dataTypes} Index:${index} Type: ${typeof dataTypes}\n`)
);
...
*/
// Write your code here
const newItems = myArr.map((Arr) => console.log(Arr));
...
*/
// Write your code here
let newCommon = student1Courses.filter((naveed) => {
  student2Courses.includes(naveed);
});
...
const newFurniture = furniture.map((naveed) => {
  console.log(`Letters in ${naveed}; `);
  for (const letters of naveed) {
    console.log(`${letters},`);
  }
});
...
const getPositiveTemperatures = temperatures.filter((num) => num > -1);
...
// Write your code here
const getOddYears = (years) => years.filter((num) => num % 2 !== 0);
